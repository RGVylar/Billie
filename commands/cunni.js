  	...
  	execute(msg, args) {
	    ...
	    if(dioce==7){
	      const exampleEmbed = new Discord.RichEmbed()
	              .setColor('#ffff00')
	              .setTitle(`You were expecting a cunni anime gif, but it was me, Dio!`)
			...
			if (!msg.channel.nsfw) {
				...
				const userlist = msg.mentions.users.map(user => {
					...
					MongoClient.connect(MONGO, function(err, db) {
				  		if (err) throw err;
				  		var dbo = db.db(DB);
				  		dbo.collection("cunni").find({}).toArray(function(err, result) {
				    		if (err) throw err;
								if(usera==userb){
								...
								return msg.channel.send(exampleEmbed);	
								    ...
								    const exampleEmbed = new Discord.RichEmbed()
									.setColor('#ffc0cb')
									.setTitle(`${resa} makes sweet love to ${resb}s pussy`)
									.setImage(gif[0]);
