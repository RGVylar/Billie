...
module.exports = {
	...
	execute(msg, args) {
		...
		if(dioce==7){
			...
			.setColor('#ffff00')
			.setTitle(`You were expecting a spank anime gif, but it was me, Dio!`)
			...
			if (!msg.channel.nsfw) {
				...
				const userlist = msg.mentions.users.map(user => {
					...
					if (args.includes('-irl')) {
		  				args.splice(args.indexOf('-irl'), 1);
						MongoClient.connect(MONGO, function(err, db) {
					  		if (err) throw err;
					  		var dbo = db.db(DB);
					  		dbo.collection("irlspank").find({}).toArray(function(err, result) {
				    			if (err) throw err;
								if(usera==userb){
						...
						if (err) throw err;
						var dbo = db.db(DB);
						dbo.collection("spank").find({}).toArray(function(err, result) {
							if (err) throw err;
							if(usera==userb){
								return msg.channel.send("Find someone else :(");
							}
